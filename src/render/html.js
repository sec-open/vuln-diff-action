...
function markdownTableToHtml(md) {
  ...
  const sep = lines[1];
  if (!sep.replace(/\|/g, "").trim().match(/^-{3,}|:?-{3,}:?/)) {
    return '<pre class="md">'+esc(md)+'</pre>';
  }
...
function writeHtmlBundle(workdir, meta /*, data not needed: bundle fetches JSONs */) {
  ...
  // 👇 OJO: sólo UNA definición de DASHBOARD_CSS
  const DASHBOARD_CSS =
