...
function buildGraphFromBOM(bomJson) {
  ...
  for (const d of dependencies) {
    ...
    ensure(adj, parentLabel);
    ensure(parents, parentLabel);
    for (const ch of (d.dependsOn || [])) {
      const childLabel = refToLabel.get(ch) || ch || "unknown";
      ensure(adj, childLabel);
      ensure(parents, childLabel);
  ...
  for (const [p, cs] of adj) {
    ensure(parents, p);
    for (const c of cs) ensure(adj, c);
  }
...
function buildMermaidGraphFromBOMImproved(bomJson, grypeMatches, graphMax = 150) {
  ...
  let idx = 0;
  const idMap = new Map();
  const idFor = (l)=>{ if(!idMap.has(l)) idMap.set(l,`n${idx++}`); return idMap.get(l); };
...
function buildMarkdownReport({ baseLabel, baseInput, baseSha, baseCommitLine, headLabel, headInput, headSha, headCommitLine, minSeverity, counts, table }) {
  const md = [];
