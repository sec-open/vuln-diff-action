...
async function renderIntro(){
  view.innerHTML = '<h2>Introduction</h2>'
    + '<div class="panel">This interactive report mirrors the PDF report, adding navigation and interactive charts. Use the left menu to browse sections.</div>';
}
...
async function renderSummary(){
  ...
  view.innerHTML = '<h2>Summary</h2>'
    ...
    + '<br/><b>Base:</b> '+esc(meta.baseLabel)+' — <code>'+esc(meta.baseSha.slice(0,12))+'</code>'
    + '<br/><b>Head:</b> '+esc(meta.headLabel)+' — <code>'+esc(meta.headSha.slice(0,12))+'</code>'
    + '<br/><b>Min severity:</b> '+esc(meta.minSeverity)
...
async function renderSeverity(){
  ...
  view.innerHTML = '<h2>Severity distribution</h2>'
    + '<div class="grid2">'
    + ' <div class="chart-box"><h3>'+esc(window.__meta__.baseLabel)+'</h3><canvas id="c1"></canvas></div>'
    + ' <div class="chart-box"><h3>'+esc(window.__meta__.headLabel)+'</h3><canvas id="c2"></canvas></div>'
    + '</div>';
...
async function renderDiff(){
  ...
  // Extract first markdown table (diff)
  const tableMd = diff.split('\\n').slice(diff.indexOf('| Severity |')>=0?diff.split('\\n').findIndex(l=>l.startsWith('| Severity |')):0).join('\\n');
  view.innerHTML = '<h2>Vulnerability diff</h2>' + mdTableToHtml(tableMd);
}

async function renderGraphBase(){
  const el = document.createElement('div');
  el.innerHTML = '<h2>Dependency graph (base)</h2><div id="m1"></div>';
...
async function renderGraphHead(){
  const el = document.createElement('div');
  el.innerHTML = '<h2>Dependency graph (head)</h2><div id="m2"></div>';
  view.innerHTML = el.innerHTML;
  const txt = await fetch('../report-landscape.html').then(r=>r.text());
...
async function renderPathsBase(){
  const html = await extractSection('../report-landscape.html', 'Dependency path base');
  view.innerHTML = '<h2>Dependency paths (base)</h2>' + html;
}
async function renderPathsHead(){
  const html = await extractSection('../report-landscape.html', 'Dependency path head');
...
// Router boot
window.__meta__ = {
  repo: "${escapeHtml(repository)}",
  baseLabel: "${escapeHtml(baseLabel)}",
  headLabel: "${escapeHtml(headLabel)}",
  minSeverity: ""
};
function route(){ const hash = location.hash || '#/intro'; navActive(); (routes[hash.slice(1)]||renderIntro)().catch(e=>{view.innerHTML='<div class="panel">Error: '+esc(e)+'</div>';}); }
